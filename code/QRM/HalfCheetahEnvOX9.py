...
import numpy as np
from DFATransformer9 import DFATransformer
...
class HalfCheetahEnv_wo_dfa(MujocoEnv, utils.EzPickle):
    ...
    def step(self, action):
        ...
        reshaped_reward = 0
        if not terminated:
            reshaped_reward = 100 / (self.automata_states_num - int(rank[self.dfa.dfa_state])) - 100 / (
                    self.automata_states_num - int(rank[last_dfa_state]))
        elif if_failure:
            reshaped_reward = 100 / (self.automata_states_num + 1) - 100 / (
                    self.automata_states_num - int(rank[last_dfa_state]))
        elif if_success:
            reshaped_reward = 100 + 100 / self.automata_states_num - 100 / (self.automata_states_num - int(rank[last_dfa_state]))
