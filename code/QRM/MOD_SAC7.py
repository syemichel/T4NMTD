...
from MySAC_OX7 import MySAC
from HalfCheetahEnv7 import HalfCheetahEnv
from HalfCheetahEnvOX7 import HalfCheetahEnvOX
import signal
import time
import sys
...
class SACOX:
    def __init__(self, domain, eval_domain, instance, classify_num, term_num, log_path, model_class=MySAC, action_noise=None):
        ...
        replay_buffers = [None for _ in range(classify_num)]
        for i, model in enumerate(self.models):
            replay_buffers[i] = model.replay_buffer

        for model in self.models:
            model.get_replay_buffers(replay_buffers)
    ...
    def learn(
        ...
        while self.train_steps < total_timesteps:
            ...
            rollout, next_dfa_state, new_states = self.models[dfa].collect_rollouts(self.models[dfa].env, callback=callback,
            ...
            if time.time() - now_time > 30:
                self.evaluate_agent(self.train_steps)
                now_time = time.time()
            self.train_steps += 1
    ...
    def predict1(
        ...
        with th.no_grad():
            actions = self.models[dfa].actor(observation, deterministic)
    ...
    def evaluate_agent(self, timestep):
        ...
        for times in range(self.eval_time):
            ...
            while not done:
                ...
                action, _ = self.predict1(state, deterministic=False, dfa=dfa)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-log', type=str, default='task1_OX_1', help='log path')
    ...
    parser.add_argument('-c', type=int, default=2, help='process num')
    ...
    agent = SACOX(domain, eval_domain, instance, classify_num=args.c, term_num=2, log_path=log_path)
