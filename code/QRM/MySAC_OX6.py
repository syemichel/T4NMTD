...
from DFATransformer6 import DFATransformer

before_accepted_state = 64
props = ['g1', 'g2', 'g3', 'g4', 'g5', 'g6', 'g7', 'g8',]
class MySAC(SAC):
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            props_ = {}
            for p in props:
                props_[p] = new_obs['p-'+p].item()
            # reward shaping and store exps
            if not dones:
                # dfa step
                for dfa_state in range(before_accepted_state):
                    self.dfa.dfa_state = str(dfa_state)
                    terminate, if_success, if_failure, next_dfa_state = self.dfa.step(props_)
