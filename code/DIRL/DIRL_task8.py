...
class PredMod:
        ...
        while n_steps < n_rollout_steps:
            ...
            if isinstance(self.lower_model.action_space, spaces.Box):
                if policy.squash_output:
                    ...
                    clipped_actions = np.clip(actions, self.lower_model.action_space.low,
                                              self.lower_model.action_space.high)
            ...
            # reward shaping
            if next_dfa_state != dfa_state and not dones:
    ...
    def evaluate(self, seconds, eval_time, goal_dfa_state):
        ...
        data = [[time_str, reward1, length / eval_time]]
        ...
        print(f'Eval reward: {reward1}, length: {length / eval_time:.2f}')
        ...
        while time.time() - self.time_start <= self.training_time:
            ...
            for neighbor in self.dfa.successors(target):
                ...
                while time.time() - now_time < 300:
                    b = self.collect_rollouts(self.lower_model.env, callback, None,
                    ...
                    if time.time() - now_time1 > 30:
                        self.evaluate(time.time() - self.time_start, eval_time=5, goal_dfa_state=self.accepting_state)
                        ...
                        if self.upper_logger[(u, v)][1] != 0:
                            self.dfa[u][v]['cost'] = -math.log(self.upper_logger[(u, v)][1])

                        if self.upper_logger[(u, v)][1] > 0.9:
                            now_time = time.time()
                            break
            ...
            for neighbor in self.dfa.successors(target):
                if (target, neighbor) in self.upper_logger and self.upper_logger[(target, neighbor)][1] > 0.9:
                    self.U_p.add(target)
