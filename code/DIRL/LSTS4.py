...
class PredMod:
    ...
    def sample_active_task(self):
        ...
        for task in self.active_tasks:
            u, v = task
            if self.state_rewards[u] < self.state_rewards[v]:
                valid_tasks.append(task)

        # 如果没有有效任务，返回None或抛出异常
        if not valid_tasks:
            return None  # 或者 raise ValueError("No valid tasks available")
        ...
        if random.random() < 0.7:
            active_task = random.choice(valid_tasks)
        else:
    ...
    def select_convergence(self, last_average_rewards, average_rewards):
        ...
        return success_rate > 0.9 and abs(average_rewards - last_average_rewards) < 10
    ...
    def learn(
        ...
        while time.time() - self.time_start <= self.training_time:
            ...
            while time.time() - now_time < 30:
                self.collect_rollouts(self.lower_model.env, callback, None,
