...
class TrainingLogger:
    def __init__(self, log_interval=10):
        ...
        self.if_successes = deque(maxlen=log_interval)
        self.current_rewards = 0
        self.current_length = 0

    def record(self, reward, done):
        if reward >= 0:
            self.current_rewards += reward.item()
        self.current_length += 1
        if done:
            self.episode_rewards.append(self.current_rewards)
            self.episode_lengths.append(self.current_length)
            if reward > 0:
                self.if_successes.append(1)
            else:
                self.if_successes.append(0)
            self.current_rewards = 0
            self.current_length = 0
    ...
    def reset(self):
        self.episode_rewards = deque(maxlen=self.log_interval)
        self.episode_lengths = deque(maxlen=self.log_interval)
        self.if_successes = deque(maxlen=self.log_interval)
        self.current_rewards = 0
        self.current_length = 0
