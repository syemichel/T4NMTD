...
from MySAC_OX import MySAC
from MyEnv_OX import MyEnv

time_start = time.time()

class SACOX:
    def __init__(self, domain, eval_domain, instance, classify_num, term_num, log_path, model_class=MySAC, action_noise=None, training_time=3600):
    ...
    def learn(
        ...
        now_time = time.time()
        while now_time - self.time_start < self.training_time:
    ...
    def evaluate_agent(self, timestep):
        ...
        for times in range(self.eval_time):
            ...
            while not done:
                ...
                # reward shaping
                dfa_state = '@q' + str(state['as'].item() + 1)
                next_dfa_state = '@q' + str(next_state['as'].item() + 1)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-r', type=str, default='racecar2', help='inst name')
    parser.add_argument('-c', type=int, default=16, help='process num')
    parser.add_argument('-t', type=int, default=3600, help='training time')
    args = parser.parse_args()
    domain = 'benchmarks/racecar/' + args.r + '.rddl'
    eval_domain = domain
    instance = 'benchmarks/racecar/' + args.i + '.rddl'
    ...
    agent = SACOX(domain, eval_domain, instance, classify_num=args.c, term_num=2, log_path=log_path, training_time=args.t)

    agent.learn(total_timesteps=2500000000)