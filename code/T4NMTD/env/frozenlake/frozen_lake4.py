...
dfa_text = '''if(ds==@q1 ^ (~p1 & ~p2 & ~p3)) then @q1 else if(ds==@q1 ^ (p3 & ~p1 & ~p2)) then @q3 else if(ds==@q1 ^ (p2 & ~p1 & ~p3)) then @q4 else if(ds==@q1 ^ ((p1 & p2) | (p1 & p3) | (p2 & p3))) then @q2 else if(ds==@q1 ^ (p1 & ~p2 & ~p3)) then @q5 else if(ds==@q3 ^ (~p1 & ~p2 & ~p3)) then @q1 else if(ds==@q3 ^ (p3 & ~p1 & ~p2 & ~p4)) then @q3 else if(ds==@q3 ^ (p2 & ~p1 & ~p3)) then @q4 else if(ds==@q3 ^ ((p1 & p2) | (p1 & p3) | (p2 & p3))) then @q2 else if(ds==@q3 ^ (p3 & p4 & ~p1 & ~p2)) then @q6 else if(ds==@q3 ^ (p1 & ~p2 & ~p3)) then @q5 else if(ds==@q4 ^ (~p1 & ~p2 & ~p3)) then @q1 else if(ds==@q4 ^ (p3 & ~p1 & ~p2)) then @q3 else if(ds==@q4 ^ (p2 & ~p1 & ~p3 & ~p4)) then @q4 else if(ds==@q4 ^ ((p1 & p2) | (p1 & p3) | (p2 & p3))) then @q2 else if(ds==@q4 ^ (p2 & p4 & ~p1 & ~p3)) then @q6 else if(ds==@q4 ^ (p1 & ~p2 & ~p3)) then @q5 else if(ds==@q2 ^ (true)) then @q2 else if(ds==@q5 ^ (~p1 & ~p2 & ~p3)) then @q1 else if(ds==@q5 ^ (p3 & ~p1 & ~p2)) then @q3 else if(ds==@q5 ^ (p2 & ~p1 & ~p3)) then @q4 else if(ds==@q5 ^ ((p1 & p2) | (p1 & p3) | (p2 & p3))) then @q2 else if(ds==@q5 ^ (p1 & ~p2 & ~p3 & ~p4)) then @q5 else if(ds==@q5 ^ (p1 & p4 & ~p2 & ~p3)) then @q6 else if(ds==@q6 ^ ((~p1 & ~p2) | (~p1 & ~p3) | (~p2 & ~p3))) then @q6 else if(ds==@q6 ^ ((p1 & p2) | (p1 & p3) | (p2 & p3))) then @q2'''
...
class FrozenLakeEnv3(FrozenLakeEnv):
    metadata = {
    ...
    def euclidean_distance(self, state1, state2):
        """计算欧几里得距离"""
        row1, col1 = self.state_to_coord(state1)
        row2, col2 = self.state_to_coord(state2)
    ...
    def step(self, a):
        ...
        terminated_dfa, if_success, if_failure = self.dfa.step(props)
        self.current_step += 1
        ...
        '''if self.dfa.dfa_state != '@q4':
            terminated = True
            reward += 0'''
        #distance = self.euclidean_distance(self.p4_loc, self.s)

        #reward -= 0.001 * distance

        '''if self.dfa.dfa_state == '@q5':
            reward = 100
            terminated = True
        elif terminated or truncated:
            reward = -10
    ...
    def reset(
        ...
        *,
        ...
        custom_state = {
            "obs": np.int32(27),  # 智能体在位置20
            "ds": np.int32(3),  # DFA状态索引2，对应@q3
            "carrying": np.int32(2),  # 手上拿着passenger 2
            "passenger_loc1": np.array([1, 7], dtype=np.int32),  # passenger 1在(1,2)
            "passenger_loc2": np.array([-1, -1], dtype=np.int32),  # passenger 2被拿在手上，位置设为(-1,-1)
            "passenger_loc3": np.array([5, 0], dtype=np.int32)  # passenger 3在(3,4)
        }

        # 4. 使用reset_from_state设置自定义状态
        reset_state, info = self.reset_from_state(custom_state)
        if self.render_mode == "human":
            self.render()
        return reset_state, info

    def reset_from_state(self, state_dict: Dict[str, Any], seed: Optional[int] = None):
        """
        从指定状态重置环境
            ...
            state_dict: 状态字典，格式为:
                {
                    ...
                    "ds": int,  # DFA状态索引
                    "carrying": int,  # 0-3
                    "passenger_loc1": np.array([row, col]),
                    "passenger_loc2": np.array([row, col]),
                    "passenger_loc3": np.array([row, col])
                }
            seed: 随机种子
        ...
        # 设置基本状态
        self.s = int(state_dict["obs"])
        self.lastaction = None
        self.current_step = 0

        # 设置DFA状态
        ds_idx = int(state_dict["ds"])
        self.dfa.dfa_state = self.dfa_states[ds_idx]

        # 设置carrying状态
        self.carrying_passenger_id = int(state_dict["carrying"])
        ...
        # 设置passenger位置
        for i in range(1, 4):
            passenger_loc = state_dict[f"passenger_loc{i}"]
            row, col = int(passenger_loc[0]), int(passenger_loc[1])
            self.passengers[i] = (row, col)
            # 只有当passenger不在手上时才设置位置映射
            if self.carrying_passenger_id != i and row >= 0 and col >= 0:
                self.passenger_locations[(row, col)] = i

        if self.render_mode == "human":
            self.render()

        current_state_dict = self._get_state_dict()
        info = {"prob": 1}

        return current_state_dict, info
