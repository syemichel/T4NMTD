...
from LowerModelPPO1 import LowerPPO, LoadLowerPPO
from stable_baselines3 import SAC, PPO
from server.ParameterServer import *
from test.EvaluatorPPO1 import SACTestAgent
...
class T4DMT:
    ...
    def run(self):
        ...
        # lower_envs[2] = GetLowerEnv(name=self.env_name, option_index=2, reset_ps=reset_ps, render_mode='human')
        ...
        upper_model_ps = upper_model_ps.remote(self.dfa, epsilon=0.3, option_num=self.option_num)
        ...
        IS_creator = IS_creator.remote(self.option_num, initial_state, 5, 15, upper_model_ps, self.dfa_text, reset_ps, eval_env)
        ...
        evaluator = evaluator.remote(policy_ps, upper_model_ps, reset_ps, self.log_path, eval_env,
                                     self.option_num, model.policy, self.dfa_text, distances)

        lower_learners = [None for _ in range(self.option_num)]
        log_path = self.log_path.rsplit('/', 1)[0]
        for i in range(self.option_num):
            file_path = log_path + '/model' + str(i) + '.zip'
            '''lower_learner = LoadLowerPPO(
                    path=log_path + '/model' + str(0),log
            ...
            if os.path.exists(file_path):
                lower_learners[i] = ray.remote(num_cpus=3)(LoadLowerPPO).remote(
                    path=log_path + '/model' + str(i),
                    policy="MultiInputPolicy",
                    env=lower_envs[i],
                    policy_ps=policy_ps,
                    reset_ps=reset_ps,
                ...
                lower_learners[i] = ray.remote(num_cpus=3)(LowerPPO).remote(
                           policy="MultiInputPolicy",
                           env=lower_envs[i],
                           policy_ps=policy_ps,
                           reset_ps=reset_ps,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-log', type=str, default='log/task1/task_transfer1.csv', help='log path')
    ...
    text_path = text_path['frozenlake2']
