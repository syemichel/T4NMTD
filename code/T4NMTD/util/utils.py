...
class UpperDiscreteEnv(gym.ActionWrapper):
    def __init__(self, env: gym.Env, option_num):
        super().__init__(env)
        self.action_space = Box(0, option_num-0.001, (1, ), dtype=np.float32)
...
def get_dfa_N(text, N):
    ...
    # 创建一个字典来存储每个边的随机分类
    edge_classes = {}
    edge_classes1 = [[] for _ in range(N)]
    edge_class = {}
    # 输出结果，排除自循环边的谓词
    for predicate, start_state, end_state in predicates:
        G.add_node(start_state)
        ...
        # 为每一条边随机分配一个类别，类别为p1, p2, ..., pn
        if start_state != end_state and end_state not in error_states:
            if predicate not in edge_class:
                edge_class[predicate] = f"p{random.randint(1, N)}"
            print(start_state, end_state, predicate, edge_class)
            edge_classes[predicate] = edge_class[predicate]
            edge_classes1[int(edge_class[predicate][-1]) - 1].append(start_state)
            print(edge_classes1)
    G.classify = edge_classes
    G.classify1 = edge_classes1
    return G
...
'''def get_dfa_N(text, N):
    ...
    # 将谓词列表打乱以实现均匀分布
    pred_list = list(valid_predicates)
    ...
    for i in range(N):
        ...
        groups.append(pred_list[start:end])
        start = end
...
class AddDFAStateObs(gym.ObservationWrapper):
    def __init__(self, env):
        ...
        self.observation_space = spaces.Dict({
            ...
            'ds': Box(low=0, high=4, shape=(1, ), dtype=np.float32)
        })
...
class MinigridActionEnv(gym.ActionWrapper):
    def __init__(self, env: gym.Env, option_num=6):
        super().__init__(env)
        self.opiton_num = option_num
