...
from HalfCheetahEnv3_lower import HalfCheetahEnv_lower
...
class HDQN:
    def __init__(self, upper_domain, lower_domain, instance, text, training_time, log_path):
        ...
        upper_env = UpperRMEnv(FrozenLakeEnv2(desc=None, map_name="8x8", is_slippery=False))
        ...
        lower_env = FrozenLakeEnvOX2(desc=None, map_name="8x8", is_slippery=False)
        eval_env = maybe_make_env(copy.deepcopy(lower_env), 1)
        self.eval_env = BaseAlgorithm._wrap_env(eval_env, 1, True)
        self.lower_model = LowerDQN("MultiInputPolicy", lower_env, verbose=1, learning_starts=500,
                                    learning_rate=3e-4, batch_size=256, train_freq=1, device='cpu')
    ...
    def evaluate(self, seconds):
        ...
        for _ in range(5):
            ...
            while not done:
                ...
                while not done:
                    action, _ = self.lower_model.predict(obs, u, v, deterministic=False)
                    ...
                    if done and reward_ < 1:
                        ...
                        reward += (0 * int(next_dfa_state == '@q1') + 10 * int(
                            next_dfa_state == '@q3' or next_dfa_state == '@q4' or next_dfa_state == '@q5') + 20 * int(
                            next_dfa_state == '@q6') + 30 * int(
                            next_dfa_state == '@q7' or next_dfa_state == '@q8' or next_dfa_state == '@q9')
                                   + 40 * int(next_dfa_state == '@q10') + 100 * int(next_dfa_state == '@q11')) - (
                                              0 * int(dfa_state == '@q1') + 10 * int(
                                          dfa_state == '@q3' or dfa_state == '@q4' or dfa_state == '@q5') + 20 * int(
                                          dfa_state == '@q6') + 30 * int(
                                          dfa_state == '@q7' or dfa_state == '@q8' or dfa_state == '@q9')
                                              + 40 * int(dfa_state == '@q10') + 100 * int(dfa_state == '@q11'))
                        break

            reward1 = max(reward1, reward.item())
        hours = int(seconds // 3600)
        minutes = int((seconds % 3600) // 60)
        second = int(seconds % 60)
        hour = str(hours) + "h" + str(minutes) + "min" + str(second) + 's'
        data = [
            [hour, reward1, length/5]
        ]
        with open(self.log_path, 'a', newline='') as csvfile:
            csvwriter = csv.writer(csvfile)
            csvwriter.writerows(data)
        print('time:', hours, "h", minutes, "min", second, 's')
        print('eval_reward:', reward1, 'eval_length:', length/5)
    ...
    def collect_rollouts(
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            assert (dfa_state == u and self.lower_model._last_obs['goal1'] == np.array([[int(u.split('q')[1]) - 1]]))

            if original_dones :
                infos[0]['terminal_observation']['obs'] = np.array([[infos[0]['terminal_observation']['obs']]])
                infos[0]['terminal_observation']['ds'] = np.array([[infos[0]['terminal_observation']['ds']]])
            ...
            if original_dones or self.current_upper_predict_N == self.upper_predict_N:
                if original_dones and rewards < 1 and next_dfa_state == '@q2':
                    ...
                    rewards_ = (0 * int(next_dfa_state == '@q1') + 10 * int(
                            next_dfa_state == '@q3' or next_dfa_state == '@q4' or next_dfa_state == '@q5') + 20 * int(
                            next_dfa_state == '@q6') + 30 * int(
                            next_dfa_state == '@q7' or next_dfa_state == '@q8' or next_dfa_state == '@q9')
                                   + 40 * int(next_dfa_state == '@q10') + 100 * int(next_dfa_state == '@q11')) - (
            ...
            if dones and next_dfa_state == v:
                ...
                rewards = np.array([[(0 * int(next_dfa_state == '@q1') + 10 * int(
                            next_dfa_state == '@q3' or next_dfa_state == '@q4' or next_dfa_state == '@q5') + 20 * int(
                            next_dfa_state == '@q6') + 30 * int(
                            next_dfa_state == '@q7' or next_dfa_state == '@q8' or next_dfa_state == '@q9')
                                   + 40 * int(next_dfa_state == '@q10') + 100 * int(next_dfa_state == '@q11')) - (
